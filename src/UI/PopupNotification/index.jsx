...
const PopupNotification = () => {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_URL}/api/notification`);
      const json = await res.json();

      const data = Array.isArray(json.notifications)
        ? json.notifications
        : [];
  ...
  return (
    <div className="fixed inset-0 z-80 flex items-center justify-center text-base-content pointer-events-none">
      ...
      {notifications.length > 0 && (
        <div
          className={`w-80 bg-white text-gray-900 border shadow-lg rounded-lg overflow-hidden transform transition-all duration-300 ease-out pointer-events-auto ${
